			...
			<div class="gap-3 mt-5 border-2 border-primary-400 ml-2">
				...
				<div class="card p-4 w-wrap shadow-xl" data-popup="popupFeatured">
					...
					<div class="flex justify-center mt-6">
						...
						<button class="bg-success-500 rounded-full ml-2 w-full" on:click={() => removeAnnotation(annotation)}>Yes</button>
					</div>
					<div class="arrow bg-surface-100-800-token" />
				</div>
				<button class="ml-2 bg-transparent text-red-500 text-2xl hover:text-white rounded-full" use:popup={popupFeatured}>X</button>
				<!-- add an id to each popup div -->
				<div id="popup-{annotation.id}" class={`card p-4 variant-filled-secondary absolute ${showPopup === annotation.id ? 'block' : 'hidden'}`}
				...
				<p class="text-xl ml-2"
					on:mouseover|stopPropagation={() => {showPopup = annotation.id;}}
					on:mouseout|stopPropagation={() => {showPopup = null;}}>
